...
customer_monthly["month"] = pd.Categorical(customer_monthly["month"].str.replace(" (kWh)", "").str.strip(), categories=months, ordered=True)
...
cust_sel = cust_sel.merge(pattern_scores, on="ACCOUNT_NUMBER", how="left").merge(zero_scores, on="ACCOUNT_NUMBER", how="left")
rel_scores = calculate_dt_relative_usage(cust_sel)
cust_sel = cust_sel.merge(rel_scores, on="ACCOUNT_NUMBER", how="left")
...
if model_choice == 'Weighted Rule-Based Model':
    ...
    score_col_name = "Weighted Probability (Avg)"
else:
